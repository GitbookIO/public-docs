---
description: Define your team's writing rules so GitBook Agent enforces them consistently.
---

# Style guide

{% hint style="info" %}
Style guides are in early access — rolling out gradually. If you don't see the feature yet, it isn't enabled for your organization.
{% endhint %}

A style guide holds your team's writing rules and conventions — the single source of truth for how content on your site should be written: voice and tone, terminology, formatting, and structure. It serves two audiences:

* **Your team** — writers and reviewers get one shared reference for how to write, so documentation stays consistent no matter who edits it.
...
Start style guide setup from two places:
...
| Template | Best for |
|---|---|
| **Starter template** | Defining your own voice and rules from scratch. Each section explains what belongs there and why, with scaffolding you fill in yourself. |
| **Google style guide** | Clear, precise, and professional writing for a developer audience. Generated from the Google developer documentation style guide. |
| **Microsoft style guide** | Warm, simple, and human writing for a broad audience — including readers who aren't technology experts. Generated from the Microsoft Writing Style Guide. |
...
**Customize the "Needs your input" sections.** Every template marks the parts you must customize with a **Needs your input** hint. Before your style guide is ready to use, work through these — they typically include:
...
If you already keep a style guide in another tool, import it instead of starting from a template:
...
GitBook imports all public pages under the URL you enter, up to 200 pages. For example, importing `website.com/docs/` includes `website.com/docs/article`, but not `website.com/other-parent/page`. For larger sites, import smaller sub-paths separately.
...
* **Content types and templates** — your page types, so writers and the Agent know which structure a page should follow. Many teams use a framework like Diátaxis.
...
**Review a page before requesting a review.** While working on a page, ask the Agent to check it against your style guide — use **Check consistency against styleguide** in the Improve menu, or ask in chat. The Agent reviews the page and leaves comments summarizing what it found.

**Request a style guide review on a change request.** When your site has a style guide, GitBook Agent appears as a suggested reviewer on your change requests, tagged **Style guide review**:
...
3. Under **Reviewers**, click **Request** next to **GitBook Agent** to have it run a style guide review of the change request. Add human reviewers alongside it, or leave the list empty to notify all reviewers in your organization.
...
The Agent never applies a rule from an upstream guide, from general knowledge of good style, or from any style guide it knows, unless the rule is written in your style guide. It also respects how your rules are written:
...
* **Exceptions are respected.** Many rules carry exceptions ("passive is fine when the actor is unknown"). Text under a listed exception isn't a violation.
...
* **Rule** — the rule ID from your style guide (e.g. `G-10`), or a short quote if it has no ID.
...
* Anything covered only by a rule that isn't in your style guide
...
A style guide complements the site-level [custom instructions](README.md#add-a-style-guide-and-custom-instructions) you can give GitBook Agent. Custom instructions are short, site-specific directions; a style guide is a full, shared document of your writing rules.
