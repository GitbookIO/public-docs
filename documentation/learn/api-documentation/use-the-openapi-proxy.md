---
description: Proxy "Test it" requests through GitBook when your API server doesn't support CORS.
---

# Use the OpenAPI proxy
...
Add `x-enable-proxy: true` at the root of your spec:
...
The proxy only forwards requests to URLs listed in your spec's `servers` array — it can't reach arbitrary URLs.

{% hint style="info" %}
Make sure your `servers` array includes every base URL you want to test against. If a URL isn't listed, "Test it" requests to that host bypass the proxy.
{% endhint %}
